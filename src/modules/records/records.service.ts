...
export class RecordsService {
  ...
  async create(createRecordDto: CreateRecordDto) {
    ...
    if (ifExist) throw new BadRequestException(`Record for hemodialysis session with id ${createRecordDto.hemodialysisSessionsId} already exist!`)
    ...
    if (afterSessions.length !== createRecordDto.afterSessions.length) throw new ConflictException('Some after session id is invalid!')
    ...
    if (homeTreatments.length !== createRecordDto.homeTreatments.length) throw new ConflictException('Some home treatment id is invalid!')
  ...
  async findAll(listParams: ListParamsDto) {
    const data = await this.recordRepository.findAll({
      ...
      populate: ['afterSessions', 'homeTreatments', 'doctor', 'afterSessions.medicine', 'homeTreatments.medicine']
    })
  ...
  async update(id: number, payload: UpdateRecordDto) {
    ...
    if (afterSessions.length !== payload.afterSessions.length) throw new ConflictException('Some after session id is invalid!')

    const homeTreatments = await this.homeTreatmentRepository.find({
      id: payload.homeTreatments
    })
    if (homeTreatments.length !== payload.homeTreatments.length) throw new ConflictException('Some home treatment id is invalid!')
